...
public class PyExampleMinitaur : MonoBehaviour {
    ...
    public Body CreateShape(b3VisualShapeData shapeData, string prefabName, Vector3 p, Quaternion qq, Vector3 scale)
    {
        GameObject prefab = null;
        for (int i = 0; i < PrimitivePrefabs.Length; i++)
        {
            if (PrimitivePrefabs[i].name == prefabName)
            {
        ...
        if (prefab == null)
        {
            Debug.LogError("Could not find a prefab named " + prefabName);
            return null;
        }
        Body bdy = new Body();
        bdy.unityProxy = GameObject.Instantiate<GameObject>(prefab).transform;
        bdy.linkIndex = shapeData.m_linkIndex;
        bdy.objectID = shapeData.m_objectUniqueId;
        bdy.unityProxy.position = p;
        bdy.unityProxy.rotation = qq;
        bdy.unityProxy.localScale = scale;
        return bdy;
    }
